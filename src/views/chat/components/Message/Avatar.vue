...
<template>
	...
	<template v-else-if="activeHistory?.icon">
		<NAvatar v-if="isDigitalPerson" :src="activeHistory?.icon"></NAvatar>
		<NAvatar v-else>
			<SvgIcon :icon="activeHistory?.icon" class="text-[26px] dark:text-white" />
		</NAvatar>
	</template>

	<span v-else class="text-[26px] dark:text-white">
